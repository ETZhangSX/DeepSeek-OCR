...
def draw_bounding_boxes(output_path, image, refs):
...
def process_image_with_refs(output_path, image, ref_texts):
    result_image = draw_bounding_boxes(output_path, image, ref_texts)
    return result_image
...
def process_single_img(image, output_path: str):
    ...
    if save_results and '<image>' in prompt:
        ...
        # print(matches_ref)
        result = process_image_with_refs(output_path, image_draw, matches_ref)
...
if __name__ == "__main__":
    ...
    images_path = glob.glob(f'{args.input_path}/*')
    ...
    for image_path in images_path:
        ...
        images.append(image)
        output_suffix = image_path.split('/')[-1].split('.')[0]
        image_output_path = os.path.join(output_path, output_suffix)
        process_single_img(image, image_output_path)
